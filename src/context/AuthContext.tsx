...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const persist = useCallback((u: User | null, verified: boolean) => {
    if (u) {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ user: u, isVerified: verified }));
    } else {
  ...
  const signup = useCallback(async (displayName: string, email: string, password: string) => {
    setStatus('loading');
    const { user: u, verified } = await mockApi.signup(displayName, email, password);
    setUser(u);
    setIsVerified(verified);
    setStatus('authenticated');
    persist(u, verified);
  ...
  return (
    <AuthContext.Provider value={{ user, isVerified, status, login, signup, verifyEmail, resendCode, logout, updateUser }}>
      {children}
